...
int main(int argc, char **argv)
{
    ...
    while(ros::ok() && wp_seq.y > wp_seq.x){ // Mientras no se haya alcanzado el ultimo waypoint
        ...
        if (wp_seq.x > 0 && wp_seq.y > wp_seq.x){ // Si no esta en el primer  waypoint y no ha alcanzado el ultimo (cuando alcanza al ultimo wp_seq.x == wp_seq.y)
            ...
            if (wp_goal.x != wp_goal_anterior_x || wp_goal.y != wp_goal_anterior_y || wp_goal.z != wp_goal_anterior_z){ // Si cambio de waypoint goal, el dron no se mueve y solo gira hasta que apunte hacia el (medida de seguridad)
                ...
                if (yaw_goal <= 30.0 ){ // Si la diferencia entre yaws es menor de 30 grados
                    wp_goal_anterior_x = wp_goal.x;
                    wp_goal_anterior_y = wp_goal.y;
                    wp_goal_anterior_z = wp_goal.z;
                }
